...
from predictor.models import RandomForestModel
...
class TestRandomForestModel:
    ...
    def setup(self, regression_data, classification_data):
        self.reg_model = RandomForestModel(task_type='regression')
        ...
        self.clf_model = RandomForestModel(task_type='classification')
    ...
    def test_grid_search_optimization_regression(self):
        param_grid = {
            'n_estimators': [50, 100],
            'max_depth': [None, 10],
        ...
        grid_search = self.reg_model.grid_search_optimization(param_grid=param_grid, cv=3)
        assert 'best_params_' in grid_search.__dict__
        assert 'best_score_' in grid_search.__dict__

    def test_grid_search_optimization_classification(self):
        param_grid = {
            'n_estimators': [50, 100],
            'max_depth': [None, 10],
            'min_samples_split': [2, 5],
        ...
        grid_search = self.clf_model.grid_search_optimization(param_grid=param_grid, cv=3)
        assert 'best_params_' in grid_search.__dict__
        assert 'best_score_' in grid_search.__dict__
